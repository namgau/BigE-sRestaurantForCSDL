...
from database.models import (User, Table, Category, Dish, Client,
                    Booking, Order, OrderedDish, Bill, TableStat, DishStat, ClientStat, HourlyStat)
...
class DatabaseDAO:
    ...
    def create_booking(self, booking: Booking):
        """Tạo đặt bàn mới qua stored procedure sp_CreateBooking (có transaction + validation)."""
        with self._get_connection() as conn:
            try:
                sql = """
                    EXEC sp_CreateBooking
                        @table_id=?, @client_id=?, @user_id=?,
                        @guest_name=?, @guest_phone=?, @guest_count=?,
                        @booking_date=?, @booking_time=?, @note=?,
                        @discount_percent=?, @booking_code=?"""
                conn.execute(sql,
                             booking.table_id, booking.client_id, booking.user_id,
    ...
    def cancel_booking(self, booking_id):
        """Hủy booking qua stored procedure sp_CancelBooking (có transaction + kiểm tra bàn còn booking khác không)."""
        with self._get_connection() as conn:
            try:
                conn.execute("EXEC sp_CancelBooking @booking_id=?", booking_id)
                conn.commit()
            except Exception:
                conn.rollback()
                raise
        ...
        with self._get_connection() as conn:
            try:
                ...
                for dish in ordered_dishes:
                    conn.execute(
                        """INSERT INTO OrderedDish(order_id,dish_id,quantity,unit_price,note)
                           VALUES(?,?,?,?,?)""",
    ...
    def pay_bill(self, bill_id, payment_method="cash"):
        """Thanh toán hóa đơn qua stored procedure sp_PayBill (có transaction đầy đủ)."""
        with self._get_connection() as conn:
            try:
                conn.execute("EXEC sp_PayBill @bill_id=?, @payment_method=?",
                             bill_id, payment_method)
                conn.commit()
            except Exception:
                conn.rollback()
                raise
        # Cập nhật thống kê vào các bảng Stat (chạy riêng sau khi bill đã paid)
